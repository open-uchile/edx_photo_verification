# -*- coding: utf-8 -*-
"""
Tests of the encryption and decryption utilities in the ssencrypt module.
"""
from __future__ import unicode_literals

from django.test import TestCase

from api.encryption.ssencrypt import *
import base64
import binascii

AES_KEY_BYTES = binascii.unhexlify(
    b"32fe72aaf2abb44de9e161131b5435c8d37cbdb6f5df242ae860b283115f2dae"
)

# Make up some garbage keys for testing purposes.
PUB_KEY_BYTES = b"""-----BEGIN PUBLIC KEY-----

EQIDAQAB
-----END PUBLIC KEY-----"""
PRIV_KEY_BYTES = b"""-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----"""


class MiddleTest(TestCase):
    def test_aes(self):
        def assert_roundtrip(data):
            """
            Verify that the original data is retrieved after encrypting and
            decrypting, and again when also using base64 encoding/decoding.
            """
            assert aes_decrypt(aes_encrypt(data, AES_KEY_BYTES), AES_KEY_BYTES) == data
            assert (
                decode_and_decrypt(
                    encrypt_and_encode(data, AES_KEY_BYTES), AES_KEY_BYTES
                )
                == data
            )

        assert_roundtrip(b"Hello World!")
        assert_roundtrip(b"1234567890123456")  # AES block size, padding corner case
        # Longer string
        assert_roundtrip(b"12345678901234561234567890123456123456789012345601")
        assert_roundtrip(b"")
        assert_roundtrip(b"\xe9\xe1a\x13\x1bT5\xc8")  # Random, non-ASCII text

    def test_rsa(self):
        self._assert_rsa(AES_KEY_BYTES, PUB_KEY_BYTES, PRIV_KEY_BYTES)

    def test_rsa_binary_data(self):
        data = b"12345678901234567890123456789012"
        self._assert_rsa(data, PUB_KEY_BYTES, PRIV_KEY_BYTES)

    def test_rsa_unicode_data_decrypt(self):
        encrypted_data = "dlPX6aF8_-_k6Ijwn-X-54rzY1x5_YtaWqcSQTVMp7SKehG5dT6nBQvVbQvKKQ6C0JvBi_aB5cjdqUvDYN67okstTpChZEVaz_aOFsg1RUWtDO64uBi6g0_OGNVKhupHDJCAXl6vOOI6tkUffuhvhDTdaucsPe5yiUun2sA5QdmcVZWsQEb_kGaGxTboxbiqpTOpyAtg0XWBLBehFgeV0nxG2UP0jzlQPMAzC2LXfjG8RWEHXQXOfWv-LW1gxNKv0UhlApN5AFtF1YiaUeR5Duka0_cGrcBb2qqUQcIWRjaV-sGNyHFjO30p8r9g0eL_W3jHpDZEXwxzb3DX2FvpCQ=="
        expect = b"12345678901234567890123456789012"
        self.assertEqual(
            rsa_decrypt(
                base64.urlsafe_b64decode(encrypted_data.encode("utf-8")),
                PRIV_KEY_BYTES.decode("utf-8"),
            ),
            expect,
        )

    def test_rsa_unicode_keys(self):
        self._assert_rsa(
            AES_KEY_BYTES, PUB_KEY_BYTES.decode("utf-8"), PRIV_KEY_BYTES.decode("utf-8")
        )

    def _assert_rsa(self, data, public_key, private_key):
        """
        Assert that the original provided data is retrieved after RSA encryption
        and decryption using the given keys.
        """
        encrypted_data = rsa_encrypt(data, public_key)
        assert rsa_decrypt(encrypted_data, private_key) == data

        # Even though our test data is only 32 bytes, RSA encryption will make it 256
        # bytes, and base64 encoding will blow that up to 344
        assert len(base64.urlsafe_b64encode(encrypted_data)) == 344
