...
def generate_aes_iv(key):
    ...
    """
    return (
        md5(key + md5(key).hexdigest().encode())
        .hexdigest()[:AES_BLOCK_SIZE_BYTES]
        .encode()
    )
...
def rsa_encrypt(data, rsa_pub_key_bytes):
    ...
    if rsa_pub_key_bytes.startswith(b"-----"):
        key = serialization.load_pem_public_key(
            rsa_pub_key_bytes, backend=default_backend()
        )
...
def rsa_decrypt(data, rsa_priv_key_bytes):
    ...
    if rsa_priv_key_bytes.startswith(b"-----"):
        key = serialization.load_pem_private_key(
            rsa_priv_key_bytes, password=None, backend=default_backend()
        )
    else:
...
def generate_signed_message(method, headers_dict, body_dict, access_key, secret_key):
    ...
    signature = binascii.b2a_base64(hashed.digest()).decode().rstrip("\n")
...
def signing_format_message(method, headers_dict, body_dict):
    ...
    """
    headers_str = "{}\n\n{}".format(method, header_string(headers_dict)).encode("utf-8")
...
def body_string(body_dict, prefix=""):
    ...
    return "".join(body_list).encode("utf-8")  # Note that trailing \n's are important
